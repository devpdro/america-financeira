...
import { Meta, Modal } from 'src/presentation/components'
...
const Intranet = () => {
  const [modal, setModal] = useState<{ isOpen: boolean; title: string; info: { text: string; link: string }[] }>({
    isOpen: false,
    title: '',
    info: [],
  })
  ...
  const handleOpenModal = (title: string, info: { text: string; link: string }[]) => {
    if (info.length === 1) {
      ...
      setModal({ isOpen: true, title, info })
    }
  }

  const handleCloseModal = () => setModal({ isOpen: false, title: '', info: [] })
  ...
  return (
    <>
      ...
      <div className={S['links-container']}>
        {data.map((item, index) => (
          <div key={index} className={S['links-box']} onClick={() => handleOpenModal(item.title, item.info)}>
      ...
      <div className={S['card-container']}>
        <div className={S['card-section']}>
          ...
          <div className={S.icons}>
            <a className={S.instagram} href="https://www.instagram.com/uiverse.io/">
